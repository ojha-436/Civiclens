 ...
 */
import { escapeHTML, validateQuestion, assistantLimiter, safeFetchJSON } from './security-utils.js';
...
export function renderAssistant(root) {
  ...
  root.innerHTML = `
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      ${['How do I register?', 'What is VVPAT?', 'Are EVMs secure?', 'What is NOTA?', 'What is the MCC?', 'How do I find my booth?']
        .map((s) => `<button class="suggest text-xs bg-civic-deep/10 text-civic-deep px-3 py-1 rounded-full hover:bg-civic-deep hover:text-white">${escapeHTML(s)}</button>`).join('')}
    </div>
  ...
  const input = /** @type {HTMLInputElement} */ (root.querySelector('#asst-input'));
  ...
  const ask = async (raw) => {
    ...
    // 1. Try local FAQ first (instant, no network)
  ...
  root.querySelector('#asst-send').addEventListener('click', () => ask(input.value));
