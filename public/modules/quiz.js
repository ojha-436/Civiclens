...
export function renderQuiz(root, questions) {
  ...
  const renderResults = () => {
    ...
    `;
    trackEvent('quiz_complete', { score, total: questions.length, tier: tier.label });
    ...
    root.querySelector('#share').addEventListener('click', async () => {
      ...
      try {
        if (navigator.share) {
          await navigator.share({ title: 'CivicLens India Quiz', text, url: location.href });
        } else {
          await navigator.clipboard.writeText(`${text} ${location.href}`);
          alert('Result copied to clipboard!');
        }
        trackEvent('quiz_shared', { method: navigator.share ? 'native' : 'clipboard' });
