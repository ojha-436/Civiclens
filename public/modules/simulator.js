 ...
 *   Walks the user through the 4-step Indian polling process:
 *   ID verification → indelible ink → EVM button → VVPAT verification,
 *   including the Rule 49MA mismatch-complaint flow.
 *
 *   All candidates shown are clearly fictional to stay within the
 *   Model Code of Conduct and ECI display guidelines.
 */
import { escapeHTML } from './security-utils.js';
import { trackEvent } from './analytics.js';

/**
 * @typedef {Object} FictionalCandidate
 * @property {string} id
...
/**
 * Render the EVM + VVPAT simulator.
 * @param {HTMLElement} root
 */
export function renderSimulator(root) {
  if (!root) return;

  const state = { step: 'id', choice: null };
  ...
  const stepRenderers = {
    id: () => `
      ...
      <button id="show-id" class="w-full bg-civic-deep text-white py-3 rounded font-semibold hover:bg-civic-accent focus:outline-none focus:ring-4 focus:ring-civic-accent/40">
    ...
    ink: () => `
      ...
      <button id="apply-ink" class="w-full bg-civic-deep text-white py-3 rounded font-semibold hover:bg-civic-accent focus:outline-none focus:ring-4 focus:ring-civic-accent/40">
        Receive ink mark →
      </button>`,
    ...
    vvpat: () => {
      ...
      return `
        ...
        <div class="mt-4 flex gap-2">
          <button id="confirm-vvpat" class="flex-1 bg-civic-deep text-white py-3 rounded font-semibold hover:bg-civic-accent focus:outline-none focus:ring-4 focus:ring-civic-accent/40">
            ✓ Slip matches my choice
          </button>
          <button id="mismatch-vvpat" class="flex-1 border-2 border-civic-accent text-civic-accent py-3 rounded font-semibold hover:bg-civic-accent hover:text-white focus:outline-none focus:ring-4 focus:ring-civic-accent/40">
            ✗ Slip does NOT match
          </button>
        </div>`;
    },
